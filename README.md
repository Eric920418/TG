...
- **雙群同步**：主群（admin-only）發訊息自動 `copyMessage` 到子群。
...
- **排程貼文**：後台建立排程 → QStash 在指定時間精準觸發 + Vercel Cron 兜底補發；支援文字、單張媒體、URL 按鈕。
- **後台**：Telegram Login Widget 登入；題庫 / 群組 / 關鍵字 / 排程 / 管理員 / 活動記錄完整 CRUD。
