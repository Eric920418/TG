...
- **手動解禁**：最簡單是點禁言通知上的「🔓 解除禁言」按鈕（僅 admin 有效）。也可由 admin **回覆**被禁言者本人的訊息打 `/unmute`，或 `/unmute <user id>`。會恢復發言權限並清空警告計數。注意：違規者的訊息會被 bot 刪光，所以**別回覆 bot 的警告訊息**打 /unmute（目標會變成 bot 自己而失敗）——這正是按鈕存在的原因。
